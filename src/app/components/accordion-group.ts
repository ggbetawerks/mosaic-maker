...
export class AccordionGroupComponent implements AfterViewInit {
  // Based on component from here: https://www.joshmorony.com/high-performance-animated-accordion-list-ionic/
  ...
  ngAfterViewInit() {
    this.blocker = this.hostElement.nativeElement.querySelector('.blocker');
  }

  @HostListener('toggle', ['$event'])
  async handleToggle(ev) {
    this.blocker.style.display = 'block';
    ev.detail.shouldOpen
      ? await this.animateOpen(ev)
      : await this.animateClose(ev);
  ...
  async animateOpen(ev) {
    ...
    const items = Array.from(this.hostElement.nativeElement.children);
    ...
    this.elementsToShift = [...items].splice(
      splitOnIndex + 1,
      items.length - (splitOnIndex + 1)
    );
